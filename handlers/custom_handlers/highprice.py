...
def highprice(message:Message) -> None:
    ...
    try:
        ...
        bot.set_state(message.from_user.id, UserHotelInfo.h_city,message.chat.id) 
        bot.send_message(message.from_user.id,f'{message.from_user.username},\nвведите город для поиска дорогих отелей (на русском языке) ')
...
@bot.message_handler(state = UserHotelInfo.h_city)
def get_city(message:Message) -> None:
    ...
    if message.text.isalpha():
        ...
        if len(city_response['suggestions'][0]['entities']) == 0:
            ...
            bot.set_state(message.from_user.id, UserHotelInfo.h_dates, message.chat.id)
            with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
def get_check_in(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, UserHotelInfo.h_check_out, message.chat.id)

    calendar_get_dates.user_calendar_one(message)


@bot.message_handler(state = UserHotelInfo.h_check_out)
    ...
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:   
            if int(message.text) > 0:
                ...
                bot.set_state(message.from_user.id, UserHotelInfo.h_img_of_hotel, message.chat.id)
...
@bot.message_handler(state = UserHotelInfo.h_img_of_hotel)
def get_img_of_hotel(message: Message) -> None:
    ...
    if message.text.isalpha():
        ...
        bot.set_state(message.from_user.id, UserHotelInfo.h_how_many_img, message.chat.id)
...
@bot.message_handler(state = UserHotelInfo.h_how_many_img)
def get_amt_of_hotel_img(message:Message) -> None:
    ...
    if message.text.isalpha():
        if message.text.lower() == 'да':
            ...
            bot.set_state(message.from_user.id, UserHotelInfo.h_get_hotel, message.chat.id)
...
@bot.message_handler(state = UserHotelInfo.h_get_hotel)
